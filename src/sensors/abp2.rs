...
///
/// ```rust,ignore
/// use cloud_chamber_firmware::sensors::abp2::Abp2Driver;
...
/// let reading = bp.read()?;
/// defmt::info!("P = {} bar, T = {} C", reading.pressure_bar, reading.temperature_c);
/// ```

use embedded_hal::i2c::I2c as I2cTrait;
use super::PressureSensor;
...
pub enum Abp2Error {
    ...
    CommandMode,
    /// La donnee lue est la meme que la precedente (conversion pas encore prete).
    StaleData,
    /// Defaut interne du capteur.
    Fault,
}
...
/// Resultat d'une mesure ABP2.
#[derive(Clone, Copy, Debug)]
pub struct Abp2Reading {
    /// Pression en bar.
    pub pressure_bar: f32,
    /// Temperature en degres Celsius (capteur integre au boitier).
...
/// Driver ABP2 generique. Instancier un `Abp2Driver` par capteur.
///
/// Le bus I2C est possede par le driver. Utiliser [`Abp2Driver::release`]
/// pour le recuperer si on veut le partager avec un autre peripherique.
...
impl<I: I2cTrait> Abp2Driver<I> {
    ...
    /// Recupere le bus I2C (permet de le partager en sequence avec d'autres drivers).
    pub fn release(self) -> I {
        self.i2c
    }

    /// Lit les 4 octets de mesure et retourne la pression et la temperature.
    ///
    /// Renvoie une erreur si le status indique stale data ou un defaut.
    pub fn read(&mut self) -> Result<Abp2Reading, Abp2Error> {
        ...
        // Temperature brute : 11 bits = [octet2 bits 7:0] concat [octet3 bits 7:5]
        let raw_t = ((buf[2] as u16) << 3) | ((buf[3] as u16) >> 5);

        // Conversion pression (interpolation lineaire sur la plage de sortie)
        ...
        // Conversion temperature : resolution 0.097 C, plage -50..+150 C
    ...
    /// Raccourci : temperature en degres Celsius uniquement.
